...
#define VERTICES_ZERO 32
#define VERTICES_ONE 6

// Declaração de várias funções utilizadas em main().  Essas estão definidas
// logo após a definição de main() neste arquivo.
void build_zero(GLfloat offset, GLfloat NDC_coefficients[VERTICES_ZERO*4]);
void build_one(GLfloat offset, GLfloat NDC_coefficients[VERTICES_ONE*4]);
GLuint BuildTriangles(GLfloat offset, bool zero, int number_of_vertices); // Constrói triângulos para renderização
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(500, 500, "INF01047 - 00301506 - Pedro Mondadori", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    //GLuint vertex_array_object_id = BuildTriangles(0.0f, true, VERTICES_ZERO);
    GLfloat spacing = 0.15f;
    GLuint vertex_array_object_id_0[4] = {
        vertex_array_object_id_0[0] = BuildTriangles(-spacing*3, true, VERTICES_ZERO),
        vertex_array_object_id_0[1] = BuildTriangles(-spacing, true, VERTICES_ZERO),
        vertex_array_object_id_0[2] = BuildTriangles(spacing, true, VERTICES_ZERO),
        vertex_array_object_id_0[3] = BuildTriangles(spacing*3, true, VERTICES_ZERO)
    };
    GLuint vertex_array_object_id_1[4]= {
        vertex_array_object_id_1[0] = BuildTriangles(-spacing*3, false, VERTICES_ONE),
        vertex_array_object_id_1[1] = BuildTriangles(-spacing, false, VERTICES_ONE),
        vertex_array_object_id_1[2] = BuildTriangles(spacing, false, VERTICES_ONE),
        vertex_array_object_id_1[3] = BuildTriangles(spacing*3, false, VERTICES_ONE)
    };
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for(int i = 1; i <= 4; i++){
            if((seconds & mask) == 0){
                glBindVertexArray(vertex_array_object_id_0[4-i]);
                glDrawElements(GL_TRIANGLE_STRIP, VERTICES_ZERO+2, GL_UNSIGNED_BYTE, 0);
                glBindVertexArray(0);
            }
            else {
                glBindVertexArray(vertex_array_object_id_1[4-i]);
                glDrawElements(GL_TRIANGLES, VERTICES_ONE+3, GL_UNSIGNED_BYTE, 0);
                glBindVertexArray(0);
            }
            mask *= 2;
        }
...
void build_zero(GLfloat offset, GLfloat NDC_coefficients[VERTICES_ZERO*16]){
    float radius_out = 0.175;
    float radius_in  = 0.125;
    float angle = 2*PI/(VERTICES_ZERO/2);

    // Pontos da circunferência
    for(int i = 0; i < VERTICES_ZERO; i++){
        if(i%2 == 0){
            NDC_coefficients[i*4] = radius_in*sin(angle*i/2)/1.7 + offset; // X
            NDC_coefficients[i*4 + 1] = radius_in*cos(angle*i/2); // Y
        }
        else{
            NDC_coefficients[i*4] = radius_out*sin(angle*i/2)/1.5 + offset; // X
            NDC_coefficients[i*4 + 1] = radius_out*cos(angle*i/2); // Y
        }
...
void build_one(GLfloat offset, GLfloat NDC_coefficients[]){
    GLfloat NDC_coefficients_aux[VERTICES_ONE*4] = {
     0.05f + offset, -0.175f, 0.0f, 1.0f, // canto direito inferior
    -0.05f + offset, -0.175f, 0.0f, 1.0f, // canto esquerdo inferior
     0.05f + offset,  0.175f, 0.0f, 1.0f, // canto direito superior
    -0.05f + offset,  0.175f, 0.0f, 1.0f, // canto esquerdo superior
    -0.05f + offset,  0.100f, 0.0f, 1.0f, //
    -0.10f + offset,  0.090f, 0.0f, 1.0f //
    };
    memcpy(NDC_coefficients, NDC_coefficients_aux, sizeof(GLfloat)*VERTICES_ONE*4);
}


// Constrói triângulos para futura renderização
GLuint BuildTriangles(GLfloat offset, bool zero, int number_of_vertices)
{
    ...
    //
    GLfloat NDC_coefficients[number_of_vertices*4];
    if(zero)
        build_zero(offset, NDC_coefficients);
    else
        build_one(offset, NDC_coefficients);
    ...
    // Conversaremos sobre sistemas de cores nas aulas de Modelos de Iluminação.
    GLfloat color_coefficients[number_of_vertices*4] = {0};

    for(int i = 0; i < number_of_vertices*4; i+=4){
        if((i%4 == 0) && zero)
    ...
    int indices_size;
    if(zero)
        indices_size = number_of_vertices+2;
    else
        indices_size = number_of_vertices + 3;

    GLubyte indices[indices_size];

    if(zero){
        for(int i = 0; i < number_of_vertices; i++){
            indices[i] = i;
        }
        indices[number_of_vertices] = 0;
        indices[number_of_vertices+1] = 1;
    }
    else{
        indices[0] = 1;
        indices[1] = 0;
